...
src/app/entry.js             existing app, followed by widget styles and panel controller
...
src/styles/widget-cockpit.css           compact control base
src/styles/widget-panels.css            attached stack and wider file/diff shell
src/styles/widget-polish.css            presentation/accessibility refinements
...
public/assets/bots/              organized bot assets under rivet, nova, butler, pixel and luma folders
...
src/app/widget-runtime.js                     existing UI, turn, microphone and speech orchestration
backend/providers/stt.py          MLX Whisper adapter
backend/providers/tts.py          persistent Kokoro ONNX engine
backend/providers/llm.py          Ollama content-streaming client
backend/conversation/chunks.py   sentence/clause-sized synthesis chunks
backend/core/app.py                  sessions, warm-up, greetings, history and narrow action tags
backend/core/memory.py               optional per-bot local JSON history
config.json                     current provider/bot/Fast-Balanced defaults
...
Start with one heavy model-generation workload at a time and one speaking session. Other bots contribute through scheduled handoffs and activity chips, not permanently loaded independent models. Fast and Balanced are the public interface; legacy High preferences migrate to Balanced when loaded.
