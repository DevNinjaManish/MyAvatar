...
7. Status banner — concise text stays on one line or within two lines, has a visible top gap below the platform, sits above chat, uses companion accent for normal updates, and uses amber only for warnings/recovery.
...
- Closed state: the status banner’s bottom edge must remain inside the 500px alert-height native window, including the widget’s 10px outer padding. With no banner, the compact shell remains 470px; showing an update reserves the extra 30px instead of clipping copy.
