...
from backend.app import app, engine_readiness
...
class DegradedWebSocket(unittest.TestCase):
    ...
    def test_tts_unavailable_keeps_text_chat_and_emits_no_audio(self):
        ...
        with TestClient(app) as client:
            ...
            with patch('backend.app.stream',fake_stream),patch('backend.app.speech.generate') as generate,client.websocket_connect('/ws?token=development') as ws:
    ...
    def test_stt_unavailable_rejects_pcm_before_transcription(self):
        with TestClient(app) as client:
            ...
            with patch('backend.app.transcribe') as transcribe,client.websocket_connect('/ws?token=development') as ws:
    ...
    def test_llm_unavailable_rejects_chat_before_model_call(self):
        ...
        with TestClient(app) as client:
            ...
            with patch('backend.app.stream',should_not_run),client.websocket_connect('/ws?token=development') as ws:
                while ws.receive_json()['type']!='ready':pass
