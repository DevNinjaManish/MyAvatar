...
test('companion configuration is complete and unique',()=>{
  ...
  for(const companion of companions){const {id}=companion;
    assert.match(id,/^[a-z][a-z0-9_-]*$/);assert.ok(companion.name);assert.ok(companion.asset);assert.equal(companion.voice?.provider,'kokoro');assert.ok(companion.voice?.voiceId);assert.ok(companion.voice?.lang);assert.ok(companion.voice?.speed>0);assert.ok(companion.voice?.name);assert.ok(companion.voice?.rate>0);assert.ok(companion.voice?.defaultEmotion);assert.ok(companion.voice?.systemPrompt);assert.ok(companion.voice?.greeting);for(const state of ['IDLE','LISTENING','THINKING','SPEAKING','WORKING'])assert.match(companion.presence?.[state]||'',/\S/);
