...
test('tools and chat resize independently and return to compact bounds',async()=>{const {window,handlers,trusted}=await boot();await handlers.get('widget-panels')(trusted,{open:true,wide:false});assert.equal(window.bounds.height,820);await handlers.get('widget-chat')(trusted,true);assert.equal(window.bounds.height,820);await handlers.get('widget-chat')(trusted,false);assert.equal(window.bounds.height,820);await handlers.get('widget-panels')(trusted,{open:false,wide:false});assert.equal(window.bounds.height,430);});
test('utility toggles preserve the anchor while changing only open height',async()=>{const {window,handlers,trusted}=await boot();const before=window.getBounds();await handlers.get('widget-chat')(trusted,true);assert.equal(window.bounds.x,before.x);assert.equal(window.bounds.height,820);await handlers.get('widget-chat')(trusted,false);assert.equal(window.bounds.x,before.x);assert.equal(window.bounds.height,430);await handlers.get('widget-panels')(trusted,{open:true,wide:true});assert.equal(window.bounds.x,before.x);assert.equal(window.bounds.height,820);await handlers.get('widget-panels')(trusted,{open:false,wide:false});assert.equal(window.bounds.x,before.x);assert.equal(window.bounds.height,430);});
...
test('full conversation remains optional and returns to compact widget',async()=>{const {handlers,trusted,window}=await boot();await handlers.get('widget-panels')(trusted,{open:true,wide:true});assert.equal(await handlers.get('window-mode')(trusted,'full'),'full');assert.equal(window.bounds.width,1120);assert.equal(await handlers.get('window-mode')(trusted,'widget'),'widget');assert.equal(window.bounds.width,WIDTH);assert.equal(window.bounds.height,430);});
