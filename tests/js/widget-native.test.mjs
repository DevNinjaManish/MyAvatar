...
test('utility toggles preserve the platform anchor while wide specialists expand left',async()=>{const {window,handlers,trusted}=await boot();const before=window.getBounds();await handlers.get('widget-chat')(trusted,true);assert.deepEqual(window.getBounds(),before);await handlers.get('widget-chat')(trusted,false);assert.deepEqual(window.getBounds(),before);await handlers.get('widget-panels')(trusted,{open:true,wide:false});const expanded=window.getBounds();assert.equal(expanded.x+450,before.x);assert.equal(expanded.width,770);await handlers.get('widget-panels')(trusted,{open:false,wide:false});assert.deepEqual(window.getBounds(),before);});
