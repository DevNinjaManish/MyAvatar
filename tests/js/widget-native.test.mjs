...
const {WIDTH,COMPACT_WIDTH,COMPACT_HEIGHT,UTILITY_HEIGHT}=layoutAPI;
...
test('chat stays compact while specialists expand independently',async()=>{const {window,handlers,trusted}=await boot();const right=window.bounds.x+window.bounds.width;await handlers.get('widget-chat')(trusted,true);assert.equal(window.bounds.width,COMPACT_WIDTH);assert.equal(window.bounds.height,COMPACT_HEIGHT);assert.equal(window.bounds.x+window.bounds.width,right);await handlers.get('widget-panels')(trusted,{open:true,wide:false});assert.equal(window.bounds.width,WIDTH);assert.equal(window.bounds.height,UTILITY_HEIGHT);assert.equal(window.bounds.x+window.bounds.width,right);await handlers.get('widget-chat')(trusted,false);assert.equal(window.bounds.width,WIDTH);await handlers.get('widget-panels')(trusted,{open:false,wide:false});assert.equal(window.bounds.width,COMPACT_WIDTH);});
test('utility toggles resize only the native hit-test frame',async()=>{const {window,handlers,trusted}=await boot();const before=window.getBounds();await handlers.get('widget-chat')(trusted,true);assert.equal(window.bounds.width,COMPACT_WIDTH);assert.equal(window.bounds.x+window.bounds.width,before.x+before.width);await handlers.get('widget-chat')(trusted,false);assert.equal(window.bounds.width,COMPACT_WIDTH);await handlers.get('widget-panels')(trusted,{open:true,wide:true});assert.equal(window.bounds.width,WIDTH);await handlers.get('widget-panels')(trusted,{open:false,wide:false});assert.equal(window.bounds.width,COMPACT_WIDTH);});
...
test('full conversation remains optional and returns to compact widget',async()=>{const {handlers,trusted,window}=await boot();await handlers.get('widget-panels')(trusted,{open:true,wide:true});assert.equal(await handlers.get('window-mode')(trusted,'full'),'full');assert.equal(window.bounds.width,1120);assert.equal(await handlers.get('window-mode')(trusted,'widget'),'widget');assert.equal(window.bounds.width,COMPACT_WIDTH);assert.equal(window.bounds.height,COMPACT_HEIGHT);});
