...
test('utility toggles never move or resize the fixed companion frame',async()=>{const {window,handlers,trusted}=await boot();const before=window.getBounds();await handlers.get('widget-chat')(trusted,true);assert.deepEqual(window.getBounds(),before);await handlers.get('widget-chat')(trusted,false);assert.deepEqual(window.getBounds(),before);await handlers.get('widget-panels')(trusted,{open:true,wide:true});assert.deepEqual(window.getBounds(),before);await handlers.get('widget-panels')(trusted,{open:false,wide:false});assert.deepEqual(window.getBounds(),before);});
