...
test('tools and chat expand independently without dropping the right-edge anchor',async()=>{const {window,handlers,trusted}=await boot();const right=window.bounds.x+window.bounds.width;await handlers.get('widget-panels')(trusted,{open:true,wide:false});assert.equal(window.bounds.height,UTILITY_HEIGHT);assert.equal(window.bounds.x+window.bounds.width,right);await handlers.get('widget-chat')(trusted,true);assert.equal(window.bounds.height,UTILITY_HEIGHT);await handlers.get('widget-chat')(trusted,false);assert.equal(window.bounds.height,UTILITY_HEIGHT);await handlers.get('widget-panels')(trusted,{open:false,wide:false});assert.equal(window.bounds.width,COMPACT_WIDTH);assert.equal(window.bounds.height,COMPACT_HEIGHT);assert.equal(window.bounds.x+window.bounds.width,right);});
test('utility toggles resize only the native hit-test frame',async()=>{const {window,handlers,trusted}=await boot();const before=window.getBounds();await handlers.get('widget-chat')(trusted,true);assert.equal(window.bounds.width,WIDTH);assert.equal(window.bounds.x+window.bounds.width,before.x+before.width);await handlers.get('widget-chat')(trusted,false);assert.equal(window.bounds.width,COMPACT_WIDTH);await handlers.get('widget-panels')(trusted,{open:true,wide:true});assert.equal(window.bounds.width,WIDTH);await handlers.get('widget-panels')(trusted,{open:false,wide:false});assert.equal(window.bounds.width,COMPACT_WIDTH);});
