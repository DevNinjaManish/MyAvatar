...
class Settings(unittest.TestCase):
        ...
        for bot in self.defaults['bots']:
            for profile in self.defaults['performanceProfiles']:
                with self.subTest(bot=bot, profile=profile):
                    ...
                    self.assertTrue(config['conversation']['system'].startswith(self.defaults['bots'][bot]['system']))
    ...
    def test_legacy_high_preference_migrates_to_balanced_and_preserves_companion_preferences(self):
        ...
        self.assertEqual(config['tts']['speed'], self.defaults['performanceProfiles']['medium']['tts']['speed'])
        ...
        self.assertEqual(json.loads(self.path.read_text())['performanceProfile'], 'medium')
    ...
    def test_bad_profile_does_not_partially_mutate_running_config(self):
        ...
        defaults['performanceProfiles']['medium']['avatar'] = []
    ...
    def test_failed_flush_keeps_old_file(self):
        self.write({'persona': 'robot'})
        original = self.path.read_bytes()
        with patch('backend.core.settings.os.fsync', side_effect=OSError('I/O')), self.assertLogs('avatar.settings', 'WARNING'):
