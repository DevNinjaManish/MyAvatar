...
export class TurnDetector {
  constructor(sampleRate,{
    ...
    Object.assign(this,{sampleRate,threshold,silenceMs,minSpeechMs,maxSpeechMs,preRollMs,onsetMs,onsetGraceMs,noiseMultiplier,noiseMargin,maxThreshold});
  ...
  push(frame){
    ...
    let sum=0,peak=0;
    for(const sample of frame){sum+=sample*sample;peak=Math.max(peak,Math.abs(sample));}
    ...
    let voiced=!transient&&rms>=this.currentThreshold;
    ...
    if(!this.started){
      ...
      if(!voiced||rms<this.currentThreshold*1.7){
        ...
        this.currentThreshold=Math.max(this.threshold,Math.min(this.maxThreshold,this.noiseFloor*this.noiseMultiplier+this.noiseMargin));
        voiced=!transient&&rms>=this.currentThreshold;
      }
    }
