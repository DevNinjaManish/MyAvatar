...
export async function runProviderStream(provider,{text,signal,onToken,timeoutMs=30000,providerOptions={}}={}){
  ...
  const providerPromise=Promise.resolve().then(()=>provider.stream({text,signal:controller.signal,...providerOptions,onToken:token=>{if(!settled)onToken?.(token);}}));
