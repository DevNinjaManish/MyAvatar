...
const FIXTURES=new Set(['compact','chat','rivet','chat-rivet','menu','picker','running','approval','complete','offline','limited']);

const taskFor=name=>{
  if(name==='running')return {id:'fixture-task',botId:'robot',goal:'Polish the companion toolbar',phase:'WORKING',status:'active',steps:[
  ...
  if(name==='approval')return {id:'fixture-task',botId:'robot',goal:'Apply approved local changes',phase:'NEEDS_APPROVAL',status:'waiting',steps:[
...
function applySurfaceState(name,doc){
  ...
  body.classList.toggle('widget-panels-open',['rivet','chat-rivet','running','approval','complete'].includes(name));
...
export function mountUiFixture(win=window,doc=document){
  ...
  else if(name==='limited')win.dispatchEvent(new win.CustomEvent('myavatar:readiness',{detail:{overall:'degraded'}}));
