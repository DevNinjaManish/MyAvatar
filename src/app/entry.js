...
$('luma-action').addEventListener('click',async()=>{const prompt=$('luma-prompt').value.trim(),edit=lumaEditing();if(!prompt){showNotice('Add a short idea or edit request first.',false,'info');return;}if(edit&&!lumaSourceImage){showNotice('Choose an image before asking Luma to edit it.',false,'info');return;}if(!desktop?.createImage){showNotice('Local image creation is available in the MyAvatar desktop app.',false,'warning');return;}const action=$('luma-action');action.disabled=true;action.textContent=edit?'Editing locally…':'Creating locally…';showNotice('Luma is working locally. The first run may take a few minutes while the model downloads.',true,'info');try{const result=await desktop.createImage({prompt,mode:edit?'edit':'generate',image:edit?lumaSourceImage:'',strength:Number($('luma-strength').value)});if(!result||result.error){showNotice(result?.error||'Luma could not create that image.',false,'warning');return;}const image=$('luma-image');image.src=result.image;image.hidden=false;$('luma-preview').classList.add('has-image');lumaSourceImage=result.image;showNotice('Created locally. Switch to Edit image to transform this result.',false,'info');}finally{action.disabled=false;action.textContent=edit?'Edit locally':'Generate locally';}});
