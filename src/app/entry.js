...
import {addRecentProject,normalizeProjectScope} from '../rivet/project-scope.js';
...
$('rivet-prepare').addEventListener('click',()=>{const request=$('rivet-request').value.trim();if(!$('rivet-project-path').dataset.path){showNotice('Choose a local folder before preparing a build plan.',false,'warning');return;}if(!request){showNotice('Describe what you would like Rivit to inspect.',false,'info');return;}showNotice('Rivit’s plan surface is ready; project inspection connects in the next capability slice.',false,'info');});
