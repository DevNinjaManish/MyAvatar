...
function setLumaMode(edit){$('luma-generate-mode').setAttribute('aria-pressed',String(!edit));$('luma-edit-mode').setAttribute('aria-pressed',String(edit));$('luma-field-kicker').textContent=edit?'IMAGE TRANSFORM':'IDEA';$('luma-field-title').textContent=edit?'What should change?':'What would you like to make?';$('luma-prompt').placeholder=edit?'Describe the edit in plain language…':'Describe an image…';$('luma-upload-label').hidden=!edit;$('luma-action').textContent=edit?'Edit locally':'Generate locally';$('luma-honesty').textContent=edit?'Your image and edit run locally. This first version transforms the whole image, not a painted selection.':'Runs locally on this Mac. First use downloads the local image model; nothing is uploaded.';}
