...
function setSpecialistOpen(open){const specialist=specialistFor[selected];for(const id of specialistIds)if(id!==specialist.id)setOpen(id,false);setOpen('more-menu',false);setOpen('companion-picker',false);setOpen(specialist.id,open);if(open&&specialist.id==='nova-workspace')renderNovaWorkspace();}
...
$('rivet-project-select').addEventListener('click',()=>{const path=window.prompt('Local folder path for Rivit to inspect',$('rivet-project-path').dataset.path||'');if(path===null)return;const clean=path.trim();if(!clean){$('rivet-project-name').textContent='Choose a folder';$('rivet-project-path').textContent='Read scope only · no project selected';$('rivet-project-path').dataset.path='';return;}$('rivet-project-name').textContent=clean.split('/').filter(Boolean).pop()||clean;$('rivet-project-path').textContent=`${clean} · read scope only`;$('rivet-project-path').dataset.path=clean;showNotice('Rivit has local read scope. Changes still need review.',false,'info');});
