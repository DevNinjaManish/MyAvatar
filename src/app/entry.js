...
import {ChatStore,shouldFollowScroll} from '../conversation/chat-store.js';
...
import {addRecentProject,clipProjectEvidence,normalizeProjectScope} from '../rivet/project-scope.js';
...
const RIVET_RECENTS_STORAGE_KEY='myavatar-rivet-recents-v1';
let novaWorkspace=loadNovaWorkspace();
let rivetRecents=loadRivetRecents();
let rivetScope=null;
let rivetFiles=[];
let rivetSelectedFile=null;
let lumaSourceImage='';
let lumaLastRequest=null;
let lumaProgressStartedAt=0;
let lumaRenderStartedAt=0;
let stopLumaProgress=null;
let lumaCancelRequested=false;
...
const specialistFor=Object.freeze({nova:{id:'nova-workspace',label:'Today',title:'Today',kicker:'NOVA · LOCAL TODAY'},sterling:{id:'nova-workspace',label:'Briefing',title:'Briefing',kicker:'STERLING · LOCAL BRIEFING'},rivet:{id:'rivet-workspace',label:'Build',title:'Build',kicker:'RIVIT · LOCAL BUILD'},luma:{id:'luma-workspace',label:'Create',title:'Create',kicker:'LUMA · LOCAL STUDIO'}});
const specialistIds=['nova-workspace','rivet-workspace','luma-workspace'];
...
function resizePanels(){const wide=specialistIds.some(id=>!$(id).hidden)||!$('more-menu').hidden;const tall=wide||!$('chat-panel').hidden;desktop?.resize?.(tall?805:550,wide?630:260);}
function setOpen(id,open){
  ...
  if(specialistIds.includes(id)){const anyOpen=specialistIds.some(panelId=>!$(panelId).hidden);$('workspace-toggle').setAttribute('aria-pressed',String(anyOpen));$('workspace-toggle').setAttribute('aria-expanded',String(anyOpen));$('workspace-toggle').setAttribute('aria-label',`${anyOpen?'Close':'Open'} ${specialistFor[selected].label}`);document.body.classList.toggle('workspace-open',anyOpen);}
...
function loadRivetRecents(){try{return JSON.parse(localStorage.getItem(RIVET_RECENTS_STORAGE_KEY)||'[]').map(normalizeProjectScope).filter(Boolean).slice(0,5);}catch{return [];}}
function saveRivetRecents(){localStorage.setItem(RIVET_RECENTS_STORAGE_KEY,JSON.stringify(rivetRecents));}
...
function renderNovaWorkspace(){const sterling=selected==='sterling';const now=new Date();$('assistant-workspace-kicker').textContent=specialistFor[selected].kicker;$('nova-workspace-title').textContent=specialistFor[selected].title;$('nova-workspace-date').textContent=new Intl.DateTimeFormat(undefined,{weekday:'long',month:'long',day:'numeric'}).format(now);$('assistant-focus-label').textContent=sterling?'Priority for today':'Today’s focus';$('nova-focus').placeholder=sterling?'What deserves your attention first?':'What matters most today?';$('assistant-reminders-title').textContent=sterling?'Outstanding items':'Keep close';$('nova-focus').value=novaWorkspace.focus;renderNovaCalendar();renderNovaList('nova-reminders',novaWorkspace.reminders,'reminder');renderNovaList('nova-events',novaWorkspace.events,'event');renderNovaList('nova-plan-list',novaWorkspace.plan,'plan');}
function updateSpecialistControl(){const specialist=specialistFor[selected];$('specialist-label').textContent=specialist.label;$('workspace-toggle').setAttribute('aria-label',`Open ${specialist.label}`);}
function setSpecialistOpen(open){const specialist=specialistFor[selected];for(const id of specialistIds)if(id!==specialist.id)setOpen(id,false);setOpen('more-menu',false);setOpen('companion-picker',false);setOpen(specialist.id,open);if(open&&specialist.id==='nova-workspace')renderNovaWorkspace();if(open&&specialist.id==='rivet-workspace')renderRivetScope(rivetScope);if(open&&specialist.id==='luma-workspace')warmLumaFast();}
async function warmLumaFast(){if(!desktop?.warmLuma)return;showNotice('Preparing Luma Fast locally…',false,'info');const result=await desktop.warmLuma();if(result?.ready)showNotice('Luma Fast is ready. It will release memory after this preflight.',false,'info');else if(result?.error)showNotice(result.error,false,'warning');}
...
function renderPerformance(metrics={},provider='unknown'){
  ...
  $('deck-memory')?.replaceChildren(document.createTextNode(memory));$('deck-context')?.replaceChildren(document.createTextNode(context));$('deck-generation')?.replaceChildren(document.createTextNode(generation));$('deck-cpu')?.replaceChildren(document.createTextNode(cpu));
...
function renderMessages(){
  ...
  const follow=shouldFollowScroll(messages);
  ...
  for(const item of items){
    const row=document.createElement('div');row.className=`message ${item.role} ${item.status}`;
    if(item.role!==previousRole||item.role==='system'){const label=document.createElement('strong');label.textContent=item.role==='user'?'You':item.role==='assistant'?chat.botName:'Status';row.append(label);}
    const text=document.createElement('span');text.textContent=item.text||item.meta?.error|| (item.status==='streaming'?'Thinking…':'');
    row.append(text);
    if(item.role==='assistant'&&item.status==='complete'&&item.text){const copy=document.createElement('button');copy.type='button';copy.className='message-copy';copy.dataset.copyText=item.text;copy.textContent='Copy';row.append(copy);}
...
chat.addEventListener('change',()=>{renderMessages();const focus=chat.focus();if(focus)localStorage.setItem('myavatar-last-topic',focus);});
 $('messages').addEventListener('click',event=>{const copy=event.target.closest('[data-copy-text]');if(copy){navigator.clipboard?.writeText(copy.dataset.copyText||'').then(()=>showNotice('Copied.',false,'info')).catch(()=>showNotice('Copy is unavailable here.',false,'warning'));return;}const button=event.target.closest('[data-retry-turn]');if(!button||activeTurn!==null)return;const text=chat.userTextForTurn(Number(button.dataset.retryTurn));$('message').value=text;$('chat-form').requestSubmit();});
window.addEventListener('myavatar:runtime-event',event=>{
  ...
  if(detail?.type==='config'){
    ...
    if(current){selected=current.id;$('companion-name').textContent=current.name;$('chat-companion').textContent=current.name.toUpperCase();stage.setAttribute('aria-label',`${current.name} avatar`);applyCompanionTheme(current);avatar.showRobot(current.id);setPresenceLine();updateSpecialistControl();$('message').value=chat.draft(current.id);$('pause-toggle').querySelector('[data-control-label]').textContent=paused?'Resume':'Pause';}
...
function describeProject(scope){if(!scope)return 'Read scope only · no project selected';const git=scope.git?.available?(scope.git.branch||'Git repository'):'No Git repository';const changes=scope.git?.changes?` · ${scope.git.changes} changed`:' · clean';return `${scope.path} · read scope only · ${git}${changes}`;}
function renderRivetFiles(){const section=$('rivet-files-section'),list=$('rivet-files');section.hidden=!rivetScope;list.replaceChildren();if(!rivetScope)return;for(const file of rivetFiles){const button=document.createElement('button');button.type='button';button.dataset.rivetFile=file.path;button.setAttribute('aria-pressed',String(rivetSelectedFile?.path===file.path));button.textContent=file.path;list.append(button);}if(!rivetFiles.length){const empty=document.createElement('span');empty.textContent='No supported text files found in the safe project list.';list.append(empty);}}
function renderRivetScope(scope=null){const current=normalizeProjectScope(scope);rivetScope=current;rivetFiles=[];rivetSelectedFile=null;$('rivet-project-name').textContent=current?.name||'Choose a folder';$('rivet-project-path').textContent=describeProject(current);$('rivet-project-path').dataset.path=current?.path||'';$('rivet-project-meta').hidden=!current;if(current)$('rivet-project-meta').textContent=`${current.fileCount}${current.hasMore?'+' :''} top-level items`;$('rivet-selected-file').textContent='No file selected.';renderRivetFiles();const recents=$('rivet-recents');recents.replaceChildren();if(!rivetRecents.length){recents.hidden=true;return;}recents.hidden=false;for(const item of rivetRecents){const button=document.createElement('button');button.type='button';button.dataset.rivetRecent=item.path;button.textContent=item.name;button.title=item.path;recents.append(button);}}
async function loadRivetFiles(){if(!rivetScope||!desktop?.listProjectFiles)return;const result=await desktop.listProjectFiles(rivetScope.path);if(!result||result.error){showNotice(result?.error||'Rivit could not list project files.',false,'warning');return;}rivetFiles=Array.isArray(result.files)?result.files:[];renderRivetFiles();}
async function inspectRivetProject(projectPath){if(!desktop?.inspectProject){showNotice('Native project inspection is available in the MyAvatar desktop app.',false,'warning');return;}const result=await desktop.inspectProject(projectPath);if(!result||result.error){showNotice(result?.error||'Rivit could not inspect that folder.',false,'warning');return;}const scope=normalizeProjectScope(result);if(!scope){showNotice('Rivit received an invalid project summary.',false,'warning');return;}rivetRecents=addRecentProject(rivetRecents,scope);saveRivetRecents();renderRivetScope(scope);await loadRivetFiles();showNotice('Rivit has local read scope. Choose one file before preparing a plan.',false,'info');}
$('rivet-project-select').addEventListener('click',async()=>{if(!desktop?.chooseProject){showNotice('Native folder selection is available in the MyAvatar desktop app.',false,'warning');return;}const projectPath=await desktop.chooseProject();if(projectPath)await inspectRivetProject(projectPath);});
$('rivet-recents').addEventListener('click',event=>{const projectPath=event.target.closest('[data-rivet-recent]')?.dataset.rivetRecent;if(projectPath)inspectRivetProject(projectPath);});
$('rivet-files').addEventListener('click',async event=>{const relativePath=event.target.closest('[data-rivet-file]')?.dataset.rivetFile;if(!relativePath||!rivetScope||!desktop?.readProjectFile)return;const result=await desktop.readProjectFile(rivetScope.path,relativePath);if(!result||result.error){showNotice(result?.error||'Rivit could not read that file.',false,'warning');return;}rivetSelectedFile={path:result.path,text:result.text};$('rivet-selected-file').textContent=`Selected: ${result.path} · ${result.text.length.toLocaleString()} characters read locally`;renderRivetFiles();});
$('rivet-prepare').addEventListener('click',()=>{const request=$('rivet-request').value.trim();if(!rivetScope){showNotice('Choose a local folder before preparing a build plan.',false,'warning');return;}if(!rivetSelectedFile){showNotice('Choose one project file as evidence first.',false,'info');return;}if(!request){showNotice('Describe what you would like Rivit to inspect.',false,'info');return;}if(!runtimeReady||activeTurn!==null||!turnPlayback.canResumeListening()||runtimeSocket?.readyState!==WebSocket.OPEN){showNotice('Rivit is waiting for the local conversation runtime.',true,'warning');return;}const evidence=clipProjectEvidence(rivetSelectedFile.text),label=evidence.truncated?' (first 24,000 characters)':'';const turn=++nextTurn,context=`Project read scope: ${rivetScope.path}\nSelected evidence file: ${rivetSelectedFile.path}${label}\n\nFile contents:\n\`\`\`\n${evidence.text}\n\`\`\`\n\nRequest: ${request}`;activeTurn=turn;appState.set(STATES.THINKING);setRuntimeStatus('Inspecting selected evidence…');chat.addUserText(request,turn);chat.applyRuntimeEvent({type:'token',turn,text:'',botId:'rivet'});$('send-message').hidden=true;$('stop-response').hidden=false;setOpen('chat-panel',true);showNotice('Rivit is preparing an evidence-backed plan in Chat.',false,'info');runtimeSocket.send(JSON.stringify({type:'turn',turn,text:context,mode:'typed',speak:liveVoiceEnabled}));});
function lumaEditing(){return $('luma-edit-mode').getAttribute('aria-pressed')==='true';}
function setLumaMode(edit){$('luma-generate-mode').setAttribute('aria-pressed',String(!edit));$('luma-edit-mode').setAttribute('aria-pressed',String(edit));$('luma-field-kicker').textContent=edit?'IMAGE TRANSFORM':'IDEA';$('luma-field-title').textContent=edit?'What should change?':'What would you like to make?';$('luma-prompt').placeholder=edit?'Describe the edit in plain language…':'Describe an image…';$('luma-upload-label').hidden=!edit;$('luma-strength-label').hidden=!edit;$('luma-action').textContent=edit?'Edit locally':'Generate locally';$('luma-honesty').textContent=edit?'Your image and edit run locally. Choose Subtle to preserve more of the original; this first version transforms the whole image, not a painted selection.':'Runs locally on this Mac. Luma adds a quality pass for cohesive composition; first use downloads the local image model.';}
$('luma-generate-mode').addEventListener('click',()=>setLumaMode(false));
$('luma-edit-mode').addEventListener('click',()=>setLumaMode(true));
$('luma-upload').addEventListener('change',event=>{const file=event.target.files?.[0];if(!file)return;if(file.size>12*1024*1024){showNotice('Choose an image smaller than 12 MB.',false,'warning');return;}const reader=new FileReader();reader.onload=()=>{lumaSourceImage=String(reader.result||'');const image=$('luma-image');image.src=lumaSourceImage;image.hidden=false;$('luma-preview').classList.add('has-image');showNotice('Image selected locally. Describe the transformation when you are ready.',false,'info');};reader.readAsDataURL(file);});
function setLumaProgress(progress){const panel=$('luma-progress'),label=$('luma-progress-label'),time=$('luma-progress-time'),fill=$('luma-progress-fill');panel.hidden=false;const elapsed=Math.max(0,Math.round((Date.now()-lumaProgressStartedAt)/1000));if(progress.stage==='rendering'){if(!lumaRenderStartedAt)lumaRenderStartedAt=Date.now();const step=Math.max(1,Number(progress.step)||1),total=Math.max(step,Number(progress.total)||22),renderElapsed=(Date.now()-lumaRenderStartedAt)/1000,remaining=Math.max(1,Math.round(renderElapsed/step*(total-step)));label.textContent=`Rendering image · ${step} / ${total}`;time.textContent=step>=total?'Finishing…':`~${remaining}s left`;fill.style.width=`${Math.min(94,Math.round(step/total*92)+4)}%`;return;}if(progress.stage==='saving'){label.textContent='Saving local creation';time.textContent=`${elapsed}s elapsed`;fill.style.width='100%';return;}label.textContent=progress.message||'Preparing Luma locally';time.textContent=progress.stage==='reclaiming'?'One moment…':elapsed>7?`${elapsed}s elapsed`:'Estimating…';fill.style.width=progress.stage==='ready'?'12%':progress.stage==='reclaiming'?'4%':'6%';}
function clearLumaProgress(){if(stopLumaProgress){stopLumaProgress();stopLumaProgress=null;}$('luma-progress').hidden=true;}
async function runLumaRequest(request){if(!desktop?.createImage)return;const action=$('luma-action'),variation=$('luma-variation'),edit=request.mode==='edit';lumaCancelRequested=false;lumaProgressStartedAt=Date.now();lumaRenderStartedAt=0;setLumaProgress({stage:'starting',message:'Starting local creation'});stopLumaProgress=desktop?.onLumaProgress?.(setLumaProgress)||null;action.disabled=true;variation.disabled=true;action.textContent=edit?'Editing locally…':'Creating locally…';showNotice('Luma is working locally.',true,'info');try{const result=await desktop.createImage(request);if(!result||result.error){if(!lumaCancelRequested)showNotice(result?.error||'Luma could not create that image.',false,'warning');return;}const image=$('luma-image');image.src=result.image;image.hidden=false;$('luma-preview').classList.add('has-image');lumaSourceImage=result.image;lumaLastRequest=request;variation.hidden=false;showNotice(`Created locally${result.seed?` · variation ${result.seed}`:''}.`,false,'info');}finally{clearLumaProgress();action.disabled=false;variation.disabled=false;action.textContent=edit?'Edit locally':'Generate locally';}}
$('luma-direct').addEventListener('click',async()=>{const prompt=$('luma-prompt').value.trim();if(!prompt){showNotice('Write an idea before refining the brief.',false,'info');return;}const button=$('luma-direct');button.disabled=true;button.textContent='Refining…';try{const result=await desktop?.directImage?.({prompt,format:$('luma-format').value});if(result?.prompt){$('luma-prompt').value=result.prompt;showNotice(result.assisted?'Luma refined the brief locally.':'Luma prepared a local structured brief.',false,'info');}else showNotice('Luma could not refine that brief.',false,'warning');}finally{button.disabled=false;button.textContent='Refine brief';}});
$('luma-action').addEventListener('click',()=>{const prompt=$('luma-prompt').value.trim(),edit=lumaEditing();if(!prompt){showNotice('Add a short idea or edit request first.',false,'info');return;}if(edit&&!lumaSourceImage){showNotice('Choose an image before asking Luma to edit it.',false,'info');return;}if(!desktop?.createImage){showNotice('Local image creation is available in the MyAvatar desktop app.',false,'warning');return;}runLumaRequest({prompt,mode:edit?'edit':'generate',image:edit?lumaSourceImage:'',strength:Number($('luma-strength').value),format:$('luma-format').value,quality:$('luma-quality').value});});
$('luma-variation').addEventListener('click',()=>{if(lumaLastRequest)runLumaRequest(lumaLastRequest);});
$('luma-cancel').addEventListener('click',async()=>{const button=$('luma-cancel');button.disabled=true;const result=await desktop?.cancelImage?.();if(result?.cancelled){lumaCancelRequested=true;showNotice('Luma creation cancelled. Nothing was uploaded.',false,'info');}else showNotice('There is no active Luma creation to cancel.',false,'info');button.disabled=false;});
...
for(const companion of companions){
  ...
  button.addEventListener('click',()=>{
    ...
    selected=companion.id;const current=companionById[selected];$('companion-name').textContent=current.name;$('chat-companion').textContent=current.name.toUpperCase();stage.setAttribute('aria-label',`${current.name} avatar`);applyCompanionTheme(current);setPresenceLine();updateSpecialistControl();
