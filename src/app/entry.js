...
function setLumaMode(edit){$('luma-generate-mode').setAttribute('aria-pressed',String(!edit));$('luma-edit-mode').setAttribute('aria-pressed',String(edit));$('luma-field-kicker').textContent=edit?'IMAGE EDIT':'IDEA';$('luma-field-title').textContent=edit?'What should change?':'What would you like to make?';$('luma-prompt').placeholder=edit?'Describe the edit in plain language…':'Describe an image…';$('luma-upload-label').hidden=!edit;$('luma-action').textContent=edit?'Prepare edit':'Generate';$('luma-honesty').textContent=edit?'Your selected image stays local. Image editing connects here next.':'Local image generation will connect here; no image is sent anywhere.';}
$('luma-generate-mode').addEventListener('click',()=>setLumaMode(false));
$('luma-edit-mode').addEventListener('click',()=>setLumaMode(true));
$('luma-upload').addEventListener('change',event=>{const file=event.target.files?.[0];if(!file)return;const image=$('luma-image');image.src=URL.createObjectURL(file);image.hidden=false;$('luma-preview').classList.add('has-image');showNotice('Image selected locally. Describe the edit when you are ready.',false,'info');});
$('luma-action').addEventListener('click',()=>{const request=$('luma-prompt').value.trim();if(!request){showNotice('Add a short idea or edit request first.',false,'info');return;}showNotice('Luma’s local creation flow is ready; the image engine connects in the next capability slice.',false,'info');});
