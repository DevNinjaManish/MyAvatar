...
async function runLumaRequest(request){if(!desktop?.createImage)return;const action=$('luma-action'),variation=$('luma-variation'),edit=request.mode==='edit';action.disabled=true;variation.disabled=true;action.textContent=edit?'Editing locally…':'Creating locally…';showNotice('Luma is working locally.',true,'info');try{const result=await desktop.createImage(request);if(!result||result.error){showNotice(result?.error||'Luma could not create that image.',false,'warning');return;}const image=$('luma-image');image.src=result.image;image.hidden=false;$('luma-preview').classList.add('has-image');lumaSourceImage=result.image;lumaLastRequest=request;variation.hidden=false;showNotice(`Created locally${result.seed?` · variation ${result.seed}`:''}.`,false,'info');}finally{action.disabled=false;variation.disabled=false;action.textContent=edit?'Edit locally':'Generate locally';}}
