...
function renderMiniCalendarEvents(events,status){const section=$('widget-mini-calendar'),list=$('widget-mini-calendar-events');if(!section||!list)return;list.innerHTML='';if(!events.length){section.dataset.calendarState='empty';list.innerHTML='<p>Your local calendar is clear.</p>';return;}section.dataset.calendarState='ready';for(const event of events.slice(0,5)){const card=document.createElement('article');card.className='mini-calendar-event';const title=document.createElement('strong');title.textContent=event.title;const meta=document.createElement('span');meta.textContent=`${event.start} · ${event.calendar}`;card.append(title,meta);list.append(card);}status.textContent=`${events.length} upcoming event${events.length===1?'':'s'} · ${localCalendarEvents.length?'Mac + local':'Mac Calendar'}`;}
async function loadMiniCalendar(){const section=$('widget-mini-calendar'),status=$('widget-mini-calendar-status'),list=$('widget-mini-calendar-events');if(!section||!status||!list)return;localCalendarEvents=readLocalCalendar();section.dataset.calendarState='loading';status.textContent='Reading calendars…';list.innerHTML='<p>Loading events…</p>';const result=await window.desktop?.getCalendarEvents?.();const nativeEvents=result?.ok?result.events||[]:[];miniCalendarEvents=calendarEvents(nativeEvents);const sourceLabel=result?.ok?'Mac Calendar + local fallback':'Local calendar fallback · Mac access unavailable';publishCalendarContext(Boolean(result?.ok)||localCalendarEvents.length>0,miniCalendarEvents.length?`${miniCalendarEvents.length} upcoming event${miniCalendarEvents.length===1?'':'s'} · ${sourceLabel}`:sourceLabel,miniCalendarEvents);renderMiniCalendar();renderMiniCalendarEvents(miniCalendarEvents,status);if(!result?.ok&&localCalendarEvents.length===0){section.dataset.calendarState='unavailable';status.textContent='Using local calendar fallback';list.innerHTML='<p class="mini-calendar-error">Mac Calendar is unavailable. Add a local event below, or allow access in System Settings.</p>';}}
 async function loadCalendarWorkspace(){
  const workspace=$('calendar-workspace'),status=$('calendar-status'),list=$('calendar-events');if(!workspace||!status||!list)return;workspace.dataset.calendarState='loading';
  if(!window.desktop?.getCalendarEvents){localCalendarEvents=readLocalCalendar();const events=calendarEvents([]);workspace.dataset.calendarState=events.length?'ready':localCalendarEvents.length?'empty':'unavailable';publishCalendarContext(localCalendarEvents.length>0,'Local calendar fallback · Mac access unavailable',events);status.textContent='Local calendar fallback · Mac access unavailable';list.replaceChildren();if(!events.length){list.innerHTML='<div class="workspace-empty"><strong>Your local calendar is clear.</strong><span>Add an event here, or connect Mac Calendar.</span></div>';return;}for(const event of events){const card=document.createElement('article');card.className='calendar-event';const title=document.createElement('strong');title.textContent=event.title;const meta=document.createElement('span');meta.textContent=`${event.start} · ${event.calendar}`;card.append(title,meta);list.append(card);}return;}
  ...
  localCalendarEvents=readLocalCalendar();const nativeEvents=result?.ok?result.events||[]:[];const events=calendarEvents(nativeEvents);workspace.dataset.calendarState=result?.ok?(events.length?'ready':'empty'):(events.length?'ready':'unavailable');publishCalendarContext(Boolean(result?.ok)||localCalendarEvents.length>0,events.length?`${events.length} upcoming event${events.length===1?'':'s'} · ${result?.ok?'Mac + local':'local fallback'}`:'Local calendar fallback · Mac access unavailable',events);status.textContent=result?.ok?'Mac Calendar + local fallback':'Local calendar fallback · Mac access unavailable';
  list.innerHTML='';
...
new MutationObserver(syncWidgetStatus).observe($('status').parentElement,{subtree:true,childList:true,attributes:true});
 function closeWidgetMenu(restoreFocus=true){$('widget-menu').hidden=true;$('widget-quality-panel').hidden=true;$('widget-system-hud').hidden=true;$('widget-quality').setAttribute('aria-expanded','false');$('widget-system-hud-toggle').setAttribute('aria-expanded','false');$('widget-more').setAttribute('aria-expanded','false');window.desktop?.widgetMenu(false);window.dispatchEvent(new CustomEvent('myavatar:system-hud',{detail:false}));if(restoreFocus)$('widget-more').focus();}
 $('widget-more').onclick=()=>{const opening=$('widget-menu').hidden;closeBotLibrary();closeWidgetMenu(false);if(opening){$('widget-menu').hidden=false;$('widget-more').setAttribute('aria-expanded','true');window.desktop?.widgetMenu(true);$('menu-close').focus();}};
