...
async function loadMiniCalendar(){const status=$('widget-mini-calendar-status'),list=$('widget-mini-calendar-events');status.textContent='Reading your Mac Calendar…';list.innerHTML='<p>Loading events…</p>';const result=await window.desktop?.getCalendarEvents?.();if(!result?.ok){publishCalendarContext(false,'Calendar permission is needed or Calendar could not be read.');status.textContent='Calendar access needs permission';list.innerHTML='<p class="mini-calendar-error">Allow MyAvatar to access Calendar in System Settings, then refresh.</p>';return;}miniCalendarEvents=result.events||[];publishCalendarContext(true,miniCalendarEvents.length?`${miniCalendarEvents.length} upcoming event${miniCalendarEvents.length===1?'':'s'} · next 14 days`:'No events in the next 14 days',miniCalendarEvents);renderMiniCalendar();status.textContent=miniCalendarEvents.length?`${miniCalendarEvents.length} upcoming event${miniCalendarEvents.length===1?'':'s'} · next 14 days`:'No events in the next 14 days';list.innerHTML='';for(const event of miniCalendarEvents.slice(0,5)){const card=document.createElement('article');card.className='mini-calendar-event';const title=document.createElement('strong');title.textContent=event.title;const meta=document.createElement('span');meta.textContent=`${event.start} · ${event.calendar}`;card.append(title,meta);list.append(card);}if(!miniCalendarEvents.length)list.innerHTML='<p>Your calendar is clear.</p>';}
 async function loadCalendarWorkspace(){
  const status=$('calendar-status'),list=$('calendar-events');
  if(!window.desktop?.getCalendarEvents){publishCalendarContext(false,'Calendar integration is unavailable in this build.');status.textContent='Calendar integration is unavailable in this build.';return;}
  ...
  if(!result?.ok){publishCalendarContext(false,'Calendar permission is needed or Calendar could not be read.');status.textContent='Calendar permission is needed or Calendar could not be read.';list.innerHTML='<div class="workspace-error"><strong>Can’t read Calendar yet.</strong><span>Allow MyAvatar to access Calendar in System Settings, then refresh.</span></div>';return;}
  const events=result.events||[];publishCalendarContext(true,events.length?`${events.length} upcoming event${events.length===1?'':'s'} · next 14 days`:'No events in the next 14 days',events);status.textContent=events.length?`${events.length} upcoming event${events.length===1?'':'s'} · next 14 days`:'No events in the next 14 days';
  list.innerHTML='';
