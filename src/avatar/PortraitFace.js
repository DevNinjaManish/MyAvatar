...
export class PortraitFace {
  constructor(bot,performance={}){
    ...
    this.texture=map;this.lastLevel=-1;this.lastBlink=-1;this.lastState='';this.lastEmotion='relaxed';this.mouthValue=0;this.imageReady=false;this.lastPaintAt=0;this.lastTime=0;this.reaction=0;this.expressionKick=0;this.nextGaze=1.8;this.gaze=0;this.gazeTarget=0;this.transitionKick=0;this.dragKick=0;
  ...
  update(t,blink,mouth,state,previousState='IDLE',transition=1,emotion='relaxed',previousEmotion='relaxed',emotionTransition=1,motion=1,attention={x:0,y:0,dragging:false}){
    if(t>this.nextGaze){this.gazeTarget=(Math.random()-.5)*.9;this.nextGaze=t+2.6+Math.random()*4.2;}
    this.gaze=THREE.MathUtils.lerp(this.gaze,this.gazeTarget,Math.min(1,.035+motion*.075));
    this.reaction=Math.max(0,this.reaction-.035);this.transitionKick=Math.max(0,this.transitionKick-.045);this.expressionKick*=.88;this.dragKick=Math.max(0,this.dragKick-.06);
    ...
    const breath=Math.sin(t*(speaking?2.65:1.35)*temperament);
    ...
    this.root.position.y=1.30+(breath*(speaking ? .011 : .006)+pose.lift+speechBeat*.006+expression.lift+this.transitionKick*.01+this.expressionKick*.006-attention.y*.008*proximity+this.dragKick*.01)*motion;
    ...
    this.root.rotation.set(
      ...
      (pose.roll+Math.sin(t*.56*temperament)*.006+(listening ? .008 : 0)+expression.roll)*motion
    );
  ...
  reactExpression(emotion,previousEmotion='relaxed'){
    this.expressionKick=emotion==='surprised'?1:emotion==='happy'?-.55:.35;
    if(emotion==='happy')this.gazeTarget=-.24;
    if(emotion==='surprised')this.gazeTarget=0;
  }
