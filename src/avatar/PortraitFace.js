...
export class PortraitFace {
  constructor(bot,performance={}){
    ...
    this.texture=map;this.lastLevel=-1;this.lastBlink=-1;this.mouthValue=0;this.imageReady=false;this.lastPaintAt=0;
  ...
  update(t,blink,mouth,state){
    ...
    if(this.imageReady&&enoughTime&&(Math.abs(this.mouthValue-this.lastLevel)>.025||Math.abs(blink-this.lastBlink)>.04))this.paintHardware(this.mouthValue,blink);
  }
  ...
  paintHardware(level,blink){
    ...
    ctx.restore();
    this.texture.needsUpdate=true;this.lastLevel=level;this.lastBlink=blink;this.lastPaintAt=performance.now();
  }
