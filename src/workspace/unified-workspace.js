...
export function agentTaskSummary(agentState={}){return compact(agentState.toolSummary||'');}
...
export function companionWorkspaceState(botId,{focus='',messages=[],draft='',calendar=EMPTY_CALENDAR}={}){
  ...
  if(botId==='pixel')return {botId,kind:'pixel',focusLabel:'Campaign objective',focus:goal||'No campaign objective captured yet',modules:[['Current message / hook',goal||'No message captured yet'],['Next marketing experiment',goal?'Choose one small experiment tied to this objective.':'State the audience or objective to shape the next experiment.'],['Copy / output',draftText?`Draft in progress · ${draftText}`:hasReply?'Latest reply is available in conversation':'No copy draft yet'],['Campaign status',hasReply?'Direction captured in conversation.':'Waiting for campaign context.']],draft:draftText,calendar:calendarState};
  if(botId==='luma')return {botId,kind:'luma',focusLabel:'Active design brief',focus:goal||'No active design brief captured yet',modules:[['Key design decision',goal?'Clarify the highest-leverage design decision.':'State the screen, audience, or design problem.'],['Critique status',hasReply?'Critique is captured in the conversation.':'No critique yet'],['Visual output',draftText?`Design notes in progress · ${draftText}`:hasReply?'Latest reply is available in conversation':'No visual output yet'],['Image tools','Not connected in this session']],draft:draftText,calendar:calendarState};
...
export function mountUnifiedWorkspace(win=window,doc=document){
  ...
  const render=()=>{
    ...
    shell.dataset.focusActive=String(Boolean(focusText()&&focusText()!=='No active focus yet'));
    ...
    taskPanel.hidden=!agentState;taskPanel.replaceChildren();if(agentState){const title=doc.createElement('span');title.textContent='Active task';const goal=doc.createElement('strong');goal.textContent=agentState.goal||'Current task';const status=doc.createElement('small');status.textContent=`${agentState.phase||'IDLE'} · ${agentState.status||'active'}`;taskPanel.append(title,goal,status);const activity=agentTaskSummary(agentState);if(activity){const detail=doc.createElement('small');detail.textContent=`Activity: ${activity}`;taskPanel.append(detail);}const steps=(agentState.steps||[]).slice(0,5);if(steps.length){const list=doc.createElement('ol');for(const step of steps){const item=doc.createElement('li');item.dataset.status=step.status||'pending';item.textContent=step.label;list.append(item);}taskPanel.append(list);}for(const [label,value,extra] of [['Observation',agentState.observation],['Verification',agentState.verification?.message||agentState.verification?.status],['Result',agentState.result]]){if(value){const detail=doc.createElement('small');detail.className=extra?'uws-task-blocker':'';detail.textContent=`${label}: ${value}`;taskPanel.append(detail);}}if(agentState.blocker){const blocker=doc.createElement('small');blocker.className='uws-task-blocker';blocker.textContent=agentState.blocker;taskPanel.append(blocker);}if(agentState.recovery?.available){const recovery=doc.createElement('small');recovery.textContent=`Recovery: ${agentState.recovery.label||'Available'}`;taskPanel.append(recovery);}}
