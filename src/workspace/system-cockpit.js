...
export function mountSystemCockpit(win=window,doc=document){
  ...
  let readiness={},config={},task=null,metrics=null,interaction=null,pollTimer=0;
  ...
  const render=()=>{
    const state=systemTelemetryState({readiness,config,task,metrics});
    const cards=[['CPU',state.cpu,state.cpu==='Unavailable'?'Local metric unavailable':'Process usage'],['Memory',state.memory,state.memoryDetail||'System metric unavailable'],['Active LLM',state.model,state.performance],['STT',state.engines.stt,'Speech recognition'],['TTS',state.engines.tts,'Speech output'],['Coding specialist',state.engines.coding,'Lazy-loaded Rivet engine'],['Current task',state.task,state.taskStatus]];
    grid.replaceChildren();
    for(const [label,value,detail] of cards){const card=doc.createElement('article');const heading=doc.createElement('span');heading.textContent=label;const valueNode=doc.createElement('strong');valueNode.textContent=value;const detailNode=doc.createElement('small');detailNode.textContent=detail;card.append(heading,valueNode,detailNode);grid.append(card);}
    status.textContent=readiness.overall==='degraded'?'Core runtime ready with limited availability':readiness.overall==='preparing'?'Preparing local engines…':'Local runtime status';
    const numeric=value=>{const match=String(value).match(/\d+/);return match?Math.max(0,Math.min(100,Number(match[0]))):0;};
    const metric=value=>{const match=String(value).match(/\d+/);return match?`${match[0]}%`:'—';};
    const railValues={cpu:metric(state.cpu),memory:metric(state.memory)};
    for(const [key,value] of Object.entries(railValues)){const node=doc.querySelector(`[data-rail-${key}]`);if(node)node.textContent=value;const bar=doc.querySelector(`[data-rail-${key}-bar]`);if(bar)bar.style.width=`${numeric(value)}%`;}
    const runtime=doc.querySelector('[data-rail-runtime]');
    if(runtime){
      const fallback=readiness.overall==='preparing'?'STARTING':readiness.overall==='degraded'?'LIMITED':readiness.overall==='unavailable'?'OFFLINE':'LOCAL';
      runtime.textContent=interaction?.runtime||fallback;
      runtime.parentElement.dataset.state=interaction?.state||readiness.overall||'unknown';
    }
    if(widgetHud){const values={cpu:state.cpu,memory:state.memory,model:state.model,stt:state.engines.stt,tts:state.engines.tts,task:state.task};for(const [key,value] of Object.entries(values)){const node=widgetHud.querySelector(`[data-system-${key}]`);if(node)node.textContent=value;}const hudStatus=widgetHud.querySelector('#widget-system-hud-status');if(hudStatus)hudStatus.textContent=interaction?.label?`Companion · ${interaction.label}`:readiness.overall==='degraded'?'Runtime ready · limited availability':readiness.overall==='preparing'?'Preparing local engines…':'Local telemetry only · no external reporting';}
  };
  ...
  const onInteraction=event=>{interaction=event.detail||null;render();};
  win.addEventListener('myavatar:workspace-mode',onMode);win.addEventListener('myavatar:runtime-event',onRuntime);win.addEventListener('myavatar:agent-task',onTask);win.addEventListener('myavatar:interaction-state',onInteraction);win.addEventListener('myavatar:system-hud',onHud);render();onMode({detail:doc.body.dataset.workspace||'coding'});
  const api={snapshot:()=>systemTelemetryState({readiness,config,task,metrics}),dispose(){clearInterval(pollTimer);win.removeEventListener('myavatar:workspace-mode',onMode);win.removeEventListener('myavatar:runtime-event',onRuntime);win.removeEventListener('myavatar:agent-task',onTask);win.removeEventListener('myavatar:interaction-state',onInteraction);win.removeEventListener('myavatar:system-hud',onHud);section.remove();delete win.__myavatarSystemCockpit;}};
