...
const createWindow=()=>{
  ...
  const win=new BrowserWindow({
    ...
    minWidth:width,minHeight:height,maxWidth:width,maxHeight:PREFERENCES_HEIGHT,
...
if(!app.requestSingleInstanceLock()) app.quit();
  ...
  ipcMain.on('window-resize',(event,height)=>{
    ...
    const nextY=Math.max(display.y,bottom-nextHeight);
    mainWindow.setBounds({x:bounds.x,y:nextY,width:260,height:nextHeight});
  });
