...
from backend.readiness import EngineReadiness
...
async def warm_models():
    ...
    try:
        import numpy as np
        index=np.arange(16000,dtype=np.float32);fixture=(np.sin(index*(2*np.pi*220/16000))*.02).astype('<f4').tobytes()
        await loop.run_in_executor(stt_pool,transcribe,fixture,config['stt'])
                ...
                async for token in stream(messages,turn_config['llm']):
                    ...
                    if not tag_checked:
                        ...
                        if action_match:
                            ...
                            if action:
                                request_id,allowed=await request_approval(approvals,action,lambda request_id:send('action_request',turn,requestId=request_id,action=action))
                                if allowed:ok=await loop.run_in_executor(None,run_action,action);await send('action_result',turn,requestId=request_id,ok=ok)
    ...
    try:
        ...
        while True:
            ...
            if kind=='turn':task=asyncio.create_task(respond(msg))
                ...
                await send('config',config=config);await send('bot_history',history=history)
                if engine_readiness.capability('speak'):greetings.request(config,reason='bot_switch')
            elif kind=='settings':
                config.setdefault('audio',{})['mode']='manual' if msg.get('interaction')=='manual' else 'live';memory_enabled=bool(msg.get('memoryEnabled',memory_enabled));config.setdefault('memory',{})['enabled']=memory_enabled;apply_profile(config,msg.get('performanceProfile'));save_preferences(config);await send('config',config=config)
                ...
                config.setdefault('audio',{})['mode']='manual' if msg.get('interaction')=='manual' else 'live';apply_profile(config,msg.get('performanceProfile'));save_preferences(config);await send('config',config=config);await send('bot_history',history=history)
