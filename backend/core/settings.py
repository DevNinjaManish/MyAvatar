...
LEGACY_PROFILE_ALIASES = {'high': 'medium'}
...
def _profile_name(value: Any, defaults: dict[str, Any], fallback: str) -> str:
    """Return a supported profile, translating only the documented legacy ID."""
    if isinstance(value, str) and value in LEGACY_PROFILE_ALIASES:
        value = LEGACY_PROFILE_ALIASES[value]
    return value if _known(value, defaults['performanceProfiles']) else fallback
    ...
    for key, valid in checks.items():
        ...
        if valid(raw[key]):
            ...
            if key == 'interaction':
                ...
                clean[key] = False
    if clean.get('performanceProfile') in LEGACY_PROFILE_ALIASES:
        clean['performanceProfile'] = LEGACY_PROFILE_ALIASES[clean['performanceProfile']]
        log.info('Migrated legacy saved performance profile to %s.', clean['performanceProfile'])
...
This avoids inheriting one profile's overrides when switching to another.
Retain the active bot, microphone mode, memory choice and greeting rotation.
The legacy High identifier is mapped to Balanced for older callers.
"""
    profiles = defaults['performanceProfiles']
    name = _profile_name(name, defaults, defaults['performanceProfile'])
...
def load_config(config_path: Path, preferences_path: Path) -> dict[str, Any]:
    ...
    raw_preferences = read_preferences(preferences_path)
    migrate_profile = isinstance(raw_preferences.get('performanceProfile'), str) and raw_preferences.get('performanceProfile') in LEGACY_PROFILE_ALIASES
    prefs = select_preferences(raw_preferences, defaults)
    ...
    if migrate_profile:
        save_preferences(config, preferences_path)
    return config
