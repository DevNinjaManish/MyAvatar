...
export class JsonWorker {
  constructor(command,args){
    ...
    const fail=error=>{this.failure=error;this.failReady(error);for(const item of this.pending.values())item.reject(error);this.pending.clear();};
    this.child.on('error',fail);this.child.on('close',()=>fail(Error('Speech recognizer stopped.')));
  ...
  request(data,timeoutMs=60000){
    ...
    return new Promise((resolve,reject)=>{
      ...
      this.pending.set(id,item);this.child.stdin.write(JSON.stringify({...data,id})+'\n',error=>{if(error){this.pending.delete(id);item.reject(error);}});
    });
  }
