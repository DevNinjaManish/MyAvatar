...
try{
  ...
  let transcript='',response='',sawAudio=false,sawConfig=false;
  socket.on('message',data=>{
    const event=JSON.parse(data);
    if(event.type==='config'){sawConfig=true;socket.send(JSON.stringify({type:'voice',turn:1,audio,mime:'audio/aiff'}));}
    if(event.type==='transcript')transcript+=event.text||'';
    if(event.type==='token')response+=event.text||'';
    if(event.type==='audio')sawAudio=Boolean(event.audio);
    if(event.type==='error'){clearTimeout(timer);console.error(event.message);process.exit(1);}
    if(event.type==='done'){clearTimeout(timer);if(!sawConfig||!transcript.trim()||!response.trim()||!sawAudio){console.error('Voice integration did not complete STT, response, and TTS.');process.exit(1);}console.log(`Voice integration passed: ${transcript.trim()} -> ${response.trim()}`);socket.close();}
  });
