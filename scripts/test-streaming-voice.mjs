...
socket.on('message',async raw=>{const event=JSON.parse(raw);if(event.type==='config'){
  ...
  socket.send(JSON.stringify({type:'voice_stream_end',streamId:'fixture'}));await new Promise(resolve=>setTimeout(resolve,1200));started=performance.now();socket.send(JSON.stringify({type:'voice',turn:1,streamId:'fixture',audio:stdout.trim(),mime:'audio/wav'}));return;
}if(event.type==='partial_transcript')partial=event.text;if(event.type==='streaming_recognition_error')streamErrors.push(event.message);if(event.type==='recognition')recognition=event;if(event.type==='done'&&event.turn===1){try{assert.equal(recognition?.streaming,true);assert.match(recognition?.text||'',/how are you/i);console.log(JSON.stringify({partial,recognition,firstTurnMs:Math.round(performance.now()-started)}));}catch(error){console.error({partial,recognition,streamErrors});console.error(error);process.exitCode=1;}clearTimeout(timer);socket.close();}});
