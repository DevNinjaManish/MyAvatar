...
const children=[];
function run(cmd,args,extra={}){const c=spawn(cmd,args,{stdio:'inherit',env:{...process.env,MYAVATAR_TOKEN:token,HF_HUB_OFFLINE:"1",MYAVATAR_WARMUP:"1",VITE_API_TOKEN:token,...extra}});children.push(c);c.on('exit',()=>stop());return c;}
...
run('.venv/bin/python',['-m','uvicorn','backend.app:app','--host','127.0.0.1','--port','8765']);
run('node',['node_modules/vite/bin/vite.js','--host','127.0.0.1','--port','5173','--strictPort']);
