...
let sawConfig=false,sawGreeting=false,sawHealth=false,sawToken=false,text='';
const timer=setTimeout(()=>{console.error('runtime integration timeout');process.exit(1);},30000);
socket.on('message',data=>{
  const event=JSON.parse(data);
  if(event.type==='config'){sawConfig=true;socket.send(JSON.stringify({type:'greeting'}));}
  ...
  if(event.type==='done'){clearTimeout(timer);if(!sawConfig||!sawGreeting||!sawHealth||!sawToken||!text.trim()){console.error('Runtime did not pass config, greeting, health, and streaming checks.');process.exit(1);}console.log(`Runtime integration passed: ${text.trim()}`);socket.close();}
